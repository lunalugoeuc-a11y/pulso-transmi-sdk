...
begin
  ...
  insert into public.prediction_evaluations as evaluation (
    prediction_id, observation_id, absolute_error, ape, evaluated_at
  )
  select prediction_id, observation_id, absolute_error, ape, now()
  from candidates
...
from public.predictions as prediction
join public.models as model on model.model_id = prediction.model_id
join public.forecast_cycles as cycle on cycle.cycle_id = prediction.cycle_id
...
join public.forecast_cycles as cycle on cycle.cycle_id = error.cycle_id
group by error.model_id, error.model_name, error.model_version,
  error.cycle_id, error.origin_at, error.station_id, cycle.horizons_minutes;
...
group by station_metric.model_id, station_metric.model_name,
  station_metric.model_version, station_metric.cycle_id, station_metric.origin_at;
...
with complete_cycles as (
  ...
  join complete_cycles as cycle
    on cycle.model_id = error.model_id and cycle.cycle_id = error.cycle_id
  ...
  group by error.model_id, error.model_name, error.model_version, error.station_id
)
select
  station.model_id,
  station.model_name,
